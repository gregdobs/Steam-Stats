...
### Fixed
- **Game artwork failing to load for the entire library.** The five "fallback" URLs for a game's cover art were five different *paths* on one host (`cdn.akamai.steamstatic.com`) — path redundancy, not host redundancy. Steam serves the same assets from three interchangeable edge providers, so whenever that one host was unreachable every fallback failed identically and every game rendered the placeholder tile at once. Fallbacks now rotate across `cloudflare`, `fastly` and `akamai` hosts *before* degrading image shape, since a host outage affects every asset for every game while a missing capsule size only affects one title. The legacy `akamai` endpoint is also no longer the default.
