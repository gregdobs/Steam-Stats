...
**This release makes Steam Stats a real desktop app.** Download it, double-click it, and it opens in its own window. No setup scripts, no console window, no browser tab.
...
Steam Stats ships as an Electron application. Double-click and it opens in its own window with its own taskbar icon — the `Start Steam Stats.bat` launcher, the console window and the browser tab are all gone. You get both an installer and a portable single-file build.

### The download stopped shipping Node twice
The previous release contained **two complete copies of the Node.js runtime** — a 75MB executable whose only job was to launch an 83MB one. That was a workaround for a packaging limitation, and 158MB of the old 165MB folder was duplicated runtime. That's gone, along with another ~75MB of dependencies and translation files that were never used.

### Steam links open in Steam
When Steam is detected on your PC, "View in Steam" on a game opens its store page **in the Steam client** instead of a browser tab — where it can actually install the game. This follows detection automatically, and there's a toggle in **Settings → Local Steam Path** if you'd rather use the web store.

The Steam API-key link during setup deliberately still opens in your normal browser, since that's where your login session and password manager are.

### Installable from a browser too
If you run from source or just prefer a browser, Chrome and Edge now offer ⋮ → **Install Steam Stats**, giving it its own window and taskbar icon without the desktop build.

### Your existing data carries over
Config, snapshot history and caches still live in `%APPDATA%\SteamStats`. Upgrading from 1.1.0 keeps everything — there's no migration step and nothing to re-enter.
