...
export default function TonightPick() {
  ...
  }, [pool, rerollsUsed, pick]);

  // First pick is free — doesn't touch the reroll budget.
  useEffect(() => {
    if (pool.length > 0 && !pick) roll(false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pool.length]);
  ...
  return (
    <section>
      ...
      <div className="ss-panel" style={{ display: 'flex', gap: 24, alignItems: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 600, letterSpacing: '-0.4px', lineHeight: 1.25 }}>
            {pick ? pick.name : '—'}
          </div>
        ...
        <div style={{ display: 'flex', gap: 10, flexShrink: 0 }}>
          <button
            onClick={() => roll(true)}
            disabled={pool.length < 2 || rerollsLeft === 0}
            style={{
              ...
              fontSize: 13.5, cursor: 'pointer', opacity: (pool.length < 2 || rerollsLeft === 0) ? 0.5 : 1,
            }}
          ...
          <button
            ...
            onMouseEnter={e => { e.currentTarget.style.background = 'linear-gradient(160deg, rgba(111,200,247,.46), rgba(111,200,247,.2))'; }}
